...
import Mergit.EventQueue.Internal (EventQueuesManager, WorkerQueue)
import Mergit.EventQueue.Internal hiding (
  EventQueuesManager (..),
  WorkerQueue (..),
 )
...
handleEventsWith ::
  ...
  -> (key -> event -> m ())
  -> m ()
handleEventsWith eventQueuesManager f = forever $
  atomicallyThenRun $ do
